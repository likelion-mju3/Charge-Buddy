...
public class ReviewSummaryService {
    ...
    private String API_KEY = "nv-473b902fde334f1da904200949e66501lg8z";

    private String URL = "https://clovastudio.stream.ntruss.com/v3/chat-completions/HCX-005" ;

    // --- 인메모리 캐시: 새 리뷰 없으면 재사용 ---
     ...
     */
    public String summarizeNow(String placeId) {
        int count = reviewRepository.countByPlaceId(placeId);
        if (count == 0) return ""; // 리뷰 없음

        LocalDateTime lastTime = reviewRepository.findLastReviewTime(placeId);
        CacheEntry cached = cache.get(placeId);
        if (cached != null
                && cached.getLastCount() == count
                && Objects.equals(cached.getLastMaxTime(), lastTime)) {
            return cached.getSummary(); // 새 리뷰 없음 → 캐시 사용
        }
        List<StationReview> recent = reviewRepository.findTop5ByPlaceIdOrderByReviewTimeDesc(placeId);
        String prompt = buildPrompt(recent);
        String summary = callClovaAndExtract(prompt);

        cache.put(placeId, new CacheEntry(lastTime, count, summary));
        return summary;
    }

    // ----- 프롬프트 구성 -----
    private String buildPrompt(List<StationReview> reviews) {
    ...
    private String callClovaAndExtract(String prompt) {
        ...
        headers.set(API_KEY, UUID.randomUUID().toString());
